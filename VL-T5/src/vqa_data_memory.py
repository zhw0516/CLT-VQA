...
class VQAFineTuneDataset(Dataset):
    def __init__(self, coco_Ours, Examplar_set, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='q_what', cates=[0,1,2]):
        ...
        for source in self.sources:
            ...
            if source == 'val':
                source = 'test'
            data_info_path = dataset_dir.joinpath(f'{source}_'+f'{task}.json')
    ...
    def __getitem__(self, idx):
        ...
        datum = self.data[idx]
        # entries = build_index(self.entries)
...
class VQADataset:
    ...
    def __init__(self, args, splits: str, verbose=True):
        ...
        with open(dataset_dir.joinpath(f'v2_mscoco_train2014_annotations.json')) as f:
            train2014_data = json.load(f)

        with open(dataset_dir.joinpath(f'v2_mscoco_val2014_annotations.json')) as f:
        ...
        for split in self.splits:
            self.data.extend(
                json.load(open(dataset_dir.joinpath("%s.json" % (split)))))
        ...
        self.ans2label = json.load(
            open(dataset_dir.joinpath("trainval_ans2label.json")))
        self.label2ans = json.load(
            open(dataset_dir.joinpath("trainval_label2ans.json")))
        args.ans2label = self.ans2label
        args.label2ans = self.label2ans
...
class VQAEvaluator:
    ...
    def evaluate_raw(self, quesid2ans: dict,is_topk_optimal=None,**kwargs):
        """https://github.com/GT-Vision-Lab/VQA/blob/master/PythonEvaluationTools/vqaEvaluation/vqaEval.py"""
