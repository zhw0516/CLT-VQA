...
from Question_type import All_task, show_results_matrix, evaluate_metric, Category_splits, ImgId_cate_map, random_dic,save_results_matrix,save_results
...
class Trainer(TrainerBase):
    def __init__(self, args, coco_Ours, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.train_dset = VQADataset(args,args.train, True)
        self.test_dset = VQADataset(args,args.test, True)
        ...
        model_kwargs = {
            ...
            'label2ans': self.train_dset.label2ans
        }
        ...
        for task_idx, task in enumerate(self.task_list[latest_task_idx+1:]):
            ...
            if args.memory:
                if task_idx != latest_task_idx + 1:
                    ...
                    data_info_path = (f'datasets/{self.task_list[task_idx - 1]}.json')
                    ...
                    print("# The size of the Memory M:", len(All_examplar))
                else:
                    All_examplar = []
                    each_memory = 0
            else:
            ...
            for idx, cateGroup in enumerate(Category_splits_random):
                ...
                for epoch in range(self.args.epochs):
                    ...
                    for now_batch in now_loader:
                        ...
                        results, lr = self.train_step(batch, epoch_results)
                        if mem_batch:
                            results_mem, lr = self.train_step(mem_batch, epoch_results)
    ...
    def train_step(self, batch, epoch_results):
        if self.args.fp16 and _use_native_amp:
            with autocast():
                if self.args.distributed:
...
def main_worker(gpu, args):
    ...
    if args.now_train:
        ...
        path = args.output + 'training_results_matrix.json'
        path1 = args.output + 'training_results.json'
        ...
        save_results(metric_dict, path1)
        ...
        try:
            path = args.output + 'testing_results_matrix.json'
            save_results_matrix(trainer.result_matrix['q_causal'], path)
            print('#------------------ Final Performance --------------------#')
            print(trainer.result_matrix['q_causal'])
            acc = 0
            for key in trainer.result_matrix['q_causal']:
                acc += trainer.result_matrix['q_causal'][key]
            print('AP:', round(acc/len(All_task), 4))
            for keys,values in zip(trainer.answers_scores.keys(), trainer.answers_scores.values()):
                print(keys + ':',values)
        except:
            pass
